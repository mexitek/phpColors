...
/**
 *
 * A view helper to aid in color lightness
 *
 * @author Arlo Carreon <arlo.carreon@bookit.com>
 *
 */
class ColorLighten extends ViewHelper {
	 ...
	 * @return string
	 */
	public function render($color = "",$lightenAmount = FALSE) {
		...
		// Darken our hsl
		$this->lightenenBy($lightenAmount);
	 ...
	 * point between current value and 1 (white).
	 * @param (int | bool) $amount
	 */
	private function lightenenBy( $amount ) {
		// Check if we were provided a number
		if( $amount ) {
			$this->_hsl['L'] = ($this->_hsl['L'] * 100) + $amount;
			$this->_hsl['L'] = ($this->_hsl['L'] > 100) ? 1:$this->_hsl['L']/100;
			
			return;
		}
		
		// We need to find out how much to darken
		$this->_hsl['L'] += ( 1 - $this->_hsl['L'])  / 2 ;
